...
<p><cfif #SelectedICJIANumber# LTE 102>Your original selection was for county-level conviction data from CHRI. CHRI conviction data by race, sex, offense type and class for most counties is not available at this level due to low numbers. However, total convictions by county are available below. Users may also want to examine the courts profile for official felony conviction totals.</cfif>A conviction is a judgment of conviction or sentence entered upon a plea of guilty or upon a verdict or finding of guilty of an offense, rendered by a legally constituted jury or by a court of competent jurisdiction authorized to try the case without a jury (<a href="http://www.ilga.gov/legislation/ilcs/ilcs5.asp?ActID=1876&ChapterID=53" target="_blank">720 ILCS 5/2-5</a>).
...
An arrest often will not lead to a conviction but can be closed though other non-conviction outcomes, such as if a state's attorney declines to prosecute the arrestee, a withheld judgement, conditional discharge or supervision, and other outcomes. The data shown in this profile are specifically those with a conviction as the outcome.
</p>
<p>
The AOIC collects data on convictions that serves as the official source of county, circuit, and statewide convictions. Users interested in official conviction data are first encouraged to examine the <a href="http://www.illinoiscourts.gov/SupremeCourt/AnnReport.asp" target="_blank">annual reports available on their website</a> and/or the courts profile available in this tool. 
...
<p>It is generally preferable to compare conviction trends across counties, circuits, or statewide by using rates of convictions per 100,000 people instead of conviction counts. Rates will take into account the population of potential arrestees. 
	Rates calculated below are per 100,000 residents in Illinois, using Census estimates. The population data used to calculate rates is available below the arrest tables at the end of this profile. 
...
<p>A court case may involve one or more charges that could lead to one or more convictions. It is difficult to determine which is the most severe charge by offense type in many convictions. For example, a conviction for a class 4 retail theft and a conviction for a class 4 criminal damage to property offense in the same court case may be classified as either of those two.
	Instead, the data below display how many convictions occurred where there was at least one conviction charge in the following categories (a description of the offenses for each category is in the <a href="#OffenseCategoriesHeading" target="_self">appendix</a>).</p>

<p>Each case that has more than one conviction may be represented multiple times in the data below if there is more than one different offense type. Therefore, the total number of convictions below will sum to a larger number than the total number of convictions elsewhere in this profile.</p>
